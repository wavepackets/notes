...
こうして最初に述べた 「基本流 (base flow)が与えられたとき、ある波長の擾乱を加えたら増幅（または減衰）されるか？」 という問題が、**「Rayleigh方程式が境界条件を満たすような固有値 $c$ と固有関数 $\tilde{v}(y)$ は何か？」**という固有値問題に帰着しました。
次ページ以降で、この問題の解析的・数値的な解法をまとめていく予定です。
